import { HardDrive, Bell, CheckCircle2 } from "lucide-react";
...
export default function SystemHealthWidget() {
  const notifications = [
    ...
    { label: "System Health Alerts", count: 0, color: "bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300" },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="border-slate-200/80 dark:border-slate-800 shadow-xs bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 transition-colors duration-200">
        <CardHeader className="border-b border-slate-100 dark:border-slate-800 pb-3">
          <CardTitle className="text-sm font-bold text-slate-900 dark:text-slate-100 flex items-center justify-between">
            ...
            <Badge className="bg-indigo-100 dark:bg-indigo-950 text-indigo-800 dark:text-indigo-300 border-indigo-200 dark:border-indigo-800 text-[10px] font-bold">
              20 Unread
            </Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="pt-3 px-4 pb-3 space-y-2">
          {notifications.map((item, idx) => (
            <div
              key={idx}
              className="flex items-center justify-between text-xs font-semibold p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/60 border border-slate-100 dark:border-slate-800"
            >
      ...
      {/* System Health & Storage Meter Widget */}
      <Card className="border-slate-200/80 dark:border-slate-800 shadow-xs bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 transition-colors duration-200">
        <CardHeader className="border-b border-slate-100 dark:border-slate-800 pb-3">
          <CardTitle className="text-sm font-bold text-slate-900 dark:text-slate-100 flex items-center justify-between">
            <span className="flex items-center gap-2">
        ...
        <CardContent className="pt-4 px-4 pb-4 space-y-3">
          {/* Storage bar */}
          <div>
            <div className="h-2.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden flex border border-slate-200/60 dark:border-slate-700">
          ...
          <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs font-semibold">
            ...
            <span className="text-emerald-700 dark:text-emerald-400 font-extrabold">99.98%</span>
          </div>
        </CardContent>
      </Card>
    </div>
